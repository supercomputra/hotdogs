...
typedef NS_ENUM(NSUInteger, FIRVisionFaceDetectorLandmark) {
  ...
  /**
   * Face landmark detection type indicating it performs contour detection for a single face (i.e.
   * the predominant face in the image). If this type is specified, then `VisionFaceDetectorMode`
   * and `VisionFaceDetectorClassification` will be ignored.
   */
  FIRVisionFaceDetectorLandmarkContour,
